...
const TouristPackages = () => {
  ...
  const threeDayPackages = {
    provadia: [
      {
        dayNumber: 1,
        bgColor: "bg-gradient-to-tr from-secondary to-primary",
        details: [
  ...
  return (
    <div className="w-full flex flex-col mt-28 md:mt-48 px-4 md:px-0 ">
      <div className="w-full md:w-2/3 flex flex-col md:flex-row mx-auto justify-center gap-10 md:gap-20 items-center ">
        <h1 className="font-body text-4xl md:text-5xl uppercase font-bold text-secondary text-center md:text-left md:hidden">
          {t("touristPackages")}
        </h1>
        ...
        <div className="w-[80%] md:text-left md:w-1/3">
          <h1 className="font-body text-3xl md:text-5xl uppercase font-bold text-secondary hidden md:block text-center md:text-left">
            {t("touristPackages")}
          </h1>
          <p className="font-body text-justify text-sm md:mt-5">
            {t("packagesDesc")}
          </p>
        </div>
      ...
      <div className="flex flex-col relative w-full md:w-2/3 overflow-x-auto hide-scrollbar md:overflow-x-clip mx-auto justify-center gap-4 mt-5">
          ...
          {sitesWithLogos.map((site) => (
            <div
              ...
              className={`relative flex flex-col items-center transition-all duration-500 ${
                selectedStation === site.key
                  ? "w-96"
                  : selectedStation
                  ? "w-40 opacity-50"
                  : "w-80"
              }`}
              onClick={() => handleStationClick(site.key)}
            >
              ...
              <img
                src={site.logo}
                alt="SiteLogo"
                className={`object-cover rounded-md w-full h-full my-4 hover:opacity-70 cursor-pointer transition-all duration-500 ${
                  selectedStation === site.key
                    ? "w-80"
                    : selectedStation
                    ? "w-40 opacity-50"
              ...
              />
              <h2
                className={`absolute bottom-5 left-0 right-0 text-center font-heading uppercase font-bold text-white transition-all duration-500 ${
                  selectedStation === site.key ? "text-3xl" : "text-xl"
                }`}
              >
                {t(site.key)}
              </h2>
        ...
        <div>
          {selectedStation && <SitesMap selectedStation={selectedStation} />}
        </div>
      </div>
      {selectedStation && (
        <div className="w-full flex flex-col items-center mt-10 ">
          <h1 className="font-body text-3xl md:text-4xl uppercase font-bold">
            {t("oneDay")}
          </h1>
          <div className="w-full md:w-2/4 text-center mx-auto">
            <p className="font-body mt-5 text-sm md:text-base">
              {t("oneDayDesc")}
            </p>
          </div>
        </div>
      ...
      <div
        className={`w-full md:w-[70%] mx-auto grid grid-cols-1 md:grid-cols-2 ${
          filteredVariants.length >= 3 ? "lg:grid-cols-3" : "lg:grid-cols-2"
        } gap-6 mt-5 md:mt-10}`}
      >
      ...
      {selectedStation && (
        <div className="w-full flex flex-col items-center mt-20 ">
          <h1 className="font-body text-3xl md:text-4xl uppercase font-bold text-primary">
            {t("twoDay")}
          </h1>
          <div className="w-full md:w-2/4 text-center mx-auto">
      ...
      <div className="w-full md:w-[70%] mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
      ...
      {selectedStation && (
        <div className="w-full flex flex-col items-center mt-20 ">
          <h1 className="font-body text-3xl md:text-4xl uppercase font-bold text-secondary">
            {t("threeDay")}
          </h1>
          <div className="w-full md:w-2/4 text-center mx-auto">
      ...
      <div className="w-full md:w-[70%] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-5 md:mt-10">
